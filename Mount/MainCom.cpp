/*
 Copyright (c) 2007-2010 nemesis Developers Association. All rights reserved.
...
class nemesisMainCom : public InemesisMainCom
{

public:
	nemesisMainCom (DWORD messageThreadId) : RefCount (0), MessageThreadId (messageThreadId)
	{
	...
	{
		if (riid == IID_IUnknown || riid == IID_InemesisMainCom)
			*ppvObject = this;
		else
		{
			*ppvObject = NULL;
...
extern "C" BOOL ComServerMain ()
{
	...
	nemesisFactory<nemesisMainCom> factory (GetCurrentThreadId ());
	DWORD cookie;

	if (IsUacSupported ())
		UacElevated = TRUE;

	if (CoRegisterClassObject (CLSID_nemesisMainCom, (LPUNKNOWN) &factory,
...
static BOOL ComGetInstance (HWND hWnd, InemesisMainCom **tcServer)
{
	return ComGetInstanceBase (hWnd, CLSID_nemesisMainCom, IID_InemesisMainCom, (void **) tcServer);
}


InemesisMainCom *GetElevatedInstance (HWND parent)
{
	InemesisMainCom *instance;
