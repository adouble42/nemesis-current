...
namespace nemesis
{
	...
	void UserInterface::CheckRequirementsForMountingVolume () const
	{
		...
		if (!Preferences.NonInteractive)
		{
			if (!SystemInfo::IsVersionAtLeast (2, 6, 24))
				ShowWarning (_("Your system uses an old version of the Linux kernel.\n\nDue to a bug in the Linux kernel, your system may stop responding when writing data to a nemesis volume. This problem can be solved by upgrading the kernel to version 2.6.24 or later."));
		}
#endif // TC_LINUX
	}
	...
	wxString UserInterface::ExceptionToMessage (const exception &ex) const
	{
		...
		size_t pos = typeName.find ("nemesis::");
		if (pos != string::npos)
		{
			return StringConverter::ToWide (typeName.substr (pos + string ("nemesis::").size()))
...
#ifdef TC_MACOSX
		EX2MSG (HigherFuseVersionRequired,			_("nemesis requires MacFUSE 1.3 or later."));
#endif
		...
		case CommandId::Help:
			{
				wstring helpText = StringConverter::ToWide (
					...
					" 1) Path to the encrypted nemesis volume.\n"
					...
					" command with option -o when a filesystem on a nemesis volume is mounted.\n"
					...
					" Specifies comma-separated mount options for a nemesis volume:\n"
					...
					"IMPORTANT:\n"
					"\n"
					"If you want to use nemesis, you must follow the security requirements and\n"
					"security precautions listed in chapter 'Security Requirements and Precautions'\n"
					"in the nemesis documentation (file 'nemesis User Guide.pdf').\n"
					"\n"
					"\nExamples:\n\n"
					"Create a new volume:\n"
					"truecrypt -t -c\n"
				...
				{
					wxDialog dialog (nullptr, wxID_ANY, _("nemesis Command Line Help"), wxDefaultPosition);
