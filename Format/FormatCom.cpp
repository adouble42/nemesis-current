/*
 Copyright (c) 2007-2008 nemesis Developers Association. All rights reserved.
...
class nemesisFormatCom : public InemesisFormatCom
{

public:
	nemesisFormatCom (DWORD messageThreadId) : RefCount (0),
	...
	{
		if (riid == IID_IUnknown || riid == IID_InemesisFormatCom)
			*ppvObject = this;
		else
		{
			*ppvObject = NULL;
...
extern "C" BOOL ComServerFormat ()
{
	...
	nemesisFactory<nemesisFormatCom> factory (GetCurrentThreadId ());
	DWORD cookie;

	if (IsUacSupported ())
		UacElevated = TRUE;

	if (CoRegisterClassObject (CLSID_nemesisFormatCom, (LPUNKNOWN) &factory,
...
static BOOL ComGetInstance (HWND hWnd, InemesisFormatCom **tcServer)
{
	return ComGetInstanceBase (hWnd, CLSID_nemesisFormatCom, IID_InemesisFormatCom, (void **) tcServer);
}


InemesisFormatCom *GetElevatedInstance (HWND parent)
{
	InemesisFormatCom *instance;
