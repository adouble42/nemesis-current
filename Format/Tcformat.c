...
// decrypt the system partition/drive and to uninstall nemesis). The function removes the system encryption key 
// data ("volume header"), the nemesis boot loader, restores the original system loader (if available),
	...
	case WM_INITDIALOG:
		{
			...
			strcat (szRescueDiskISO, "\\nemesis Rescue Disk.iso");
...
retryCDDriveCheck:
			...
			else if (nCurPageNo == SYSENC_PRETEST_INFO_PAGE)
			{
				...
				/* Add the main nemesis app to the system startup sequence (the nemesis Background Task), which
				we need e.g. for notifications about prevented hibernation, about hidden OS leak protection, about 
				inconsistent hidden OS installs (nemesis upgraded in the decoy system but not in the hidden one), etc.
...
// the nemesis volume format version and the type of volume.
int AnalyzeHiddenVolumeHost (HWND hwndDlg, int *driveNo, __int64 hiddenVolHostSize, int *realClusterSize, __int64 *pnbrFreeClusters)
{
	...
	{
		// We do not support creating hidden volumes within volumes created by nemesis 5.1a or earlier.
