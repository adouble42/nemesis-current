	...
	while (true)
	{
		...
		if (++incorrectPasswordCount == 4)
		{
			...
			Print ("If you are sure the password is correct, the key data may be damaged. Boot your\r\n"
				   "nemesis Rescue Disk and select 'Repair Options' > 'Restore key data'.\r\n\r\n");
#endif
		}
	}
...
static void DecryptDrive (byte drive)
{
	...
	Print ("\r\nUse only if Windows cannot start. Decryption under Windows is much faster\r\n"
			"(in nemesis, select 'System' > 'Permanently Decrypt').\r\n\r\n");
...
static void RepairMenu ()
{
	...
	while (true)
	{
		...
		enum
		{
			RestoreNone = 0,
			DecryptVolume,
			RestorenemesisLoader,
			RestoreVolumeHeader,
			RestoreOriginalSystemLoader
		};

		static const char *options[] = { "Permanently decrypt system partition/drive", "Restore nemesis Boot Loader", "Restore key data (volume header)", "Restore original system loader" };
			...
			i < TC_BOOT_LOADER_AREA_SECTOR_COUNT; ++i)
		{
			...
			else if (selection == RestorenemesisLoader)
			...
			{
				if (selection == RestorenemesisLoader)
					continue;
		...
		switch (selection)
		{
		case RestorenemesisLoader:
			Print ("nemesis Boot Loader");
			break;
